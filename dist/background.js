...
// src/background/shims/node-empty.js
var node_empty_exports = {};
__export(node_empty_exports, {
  default: () => node_empty_default,
...
var notAvailable, readFileSync, promises, existsSync, join, resolve, node_empty_default;
var init_node_empty = __esm({
  "src/background/shims/node-empty.js"() {
    notAvailable = () => {
      throw new Error("Node built-in modules are not available in the extension service worker");
    };
    readFileSync = notAvailable;
    promises = new Proxy({}, { get: notAvailable });
    existsSync = () => false;
    join = notAvailable;
...
var DEFAULTS = {
  ...
  model: "claude-opus-5",
  baseUrl: "",
  // only used by the "custom" provider (and to override ollama's port)
  effort: "high",
  // Anthropic only: low | medium | high | xhigh | max
  maxSteps: 25,
  // hard ceiling on tool-calling rounds per request
  vision: false,
  // expose the screenshot tool
  confirmActions: false,
  // ask in the side panel before every action
  askWhereToSave: false,
  // show the browser's "Save as" dialog for downloads
  blockedHosts: [],
  // hostnames the agent must not act on
  maxElements: 150,
  // interactive elements per page snapshot
  maxPageText: 4e3
  // characters of page text per snapshot
};
var PROVIDERS = {
  anthropic: {
...
var CONTENT_SCRIPT = "src/content/content.js";
var elementMaps = /* @__PURE__ */ new Map();
function forgetTab(tabId) {
  elementMaps.delete(tabId);
}
async function resolveTab(session) {
  ...
  const [tab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true }) || await chrome.tabs.query({ active: true });
  if (!tab) throw new Error("No open tab to work with \u2014 open a page first");
  session.tabId = tab.id;
  return tab;
}
function isControllable(url) {
...
function waitForLoad(tabId, timeout = 15e3) {
...
async function listFrames(tabId) {
  ...
  return frames.filter((f) => !f.url || isControllable(f.url)).sort((a, b) => a.frameId - b.frameId);
}
...
function formatElement(index, el, inFrame) {
  ...
  if (el.checked !== void 0) bits.push(el.checked ? "checked" : "unchecked");
...
async function snapshot(tabId, { maxElements = 150, maxPageText = 4e3 } = {}) {
  const tab = await chrome.tabs.get(tabId);
  if (!isControllable(tab.url)) {
    throw new Error(
      `This page cannot be controlled by an extension (${tab.url || "unknown URL"}). Chrome blocks extensions on chrome:// pages, the Web Store and other browsers' internal pages. Navigate to a normal website first.`
    );
  }
...
function renderSnapshot(snap, { heading = "Current page" } = {}) {
...
var sleep2 = (ms, signal) => new Promise((resolve2) => {
  if (signal?.aborted)
    return resolve2();
  const onAbort = () => {
    clearTimeout(timer);
...
async function checkCredentialsFileSafety(path2, onWarn = (m) => console.warn(`anthropic-sdk: ${m}`)) {
  if (typeof process === "undefined" || process.platform === "win32")
    return;
  const fs = await Promise.resolve().then(() => (init_node_empty(), node_empty_exports));
  let resolved = path2;
  let st;
  try {
...
async function writeCredentialsFileAtomic(targetPath, data) {
  const fs = await Promise.resolve().then(() => (init_node_empty(), node_empty_exports));
  const path2 = await Promise.resolve().then(() => (init_node_empty(), node_empty_exports));
    ...
    switch (event.type) {
      ...
      case "message_delta":
        ...
        if (event.usage.fallback_credit != null) {
          snapshot2.usage.fallback_credit = event.usage.fallback_credit;
        }
        return snapshot2;
      case "content_block_start":
...
// src/background/llm/anthropic.js
var ADAPTIVE_THINKING = /^claude-(opus-5|opus-4-[678]|sonnet-5|sonnet-4-6|fable-5|mythos-5)/;
function toContent(message) {
  if (message.role === "user") {
    ...
    blocks2.push({ type: "text", text: message.text || "" });
    return blocks2;
  }
...
async function chat({ config, system, messages, tools, signal }) {
...
async function chat2({ config, system, messages, tools, signal }) {
  const baseUrl = resolveBaseUrl(config);
...
function toContents(messages) {
  ...
  for (const message of messages) {
    if (message.role === "assistant") {
      const parts2 = [];
      if (message.text) parts2.push({ text: message.text });
      for (const call of message.toolCalls || []) {
        parts2.push({ functionCall: { name: call.name, args: call.input || {} } });
      }
...
async function chat3({ config, system, messages, tools, signal }) {
  const baseUrl = resolveBaseUrl(config);
  ...
  const toolCalls = parts.filter((part) => part.functionCall).map((part, i) => {
...
// src/background/llm/index.js
var ADAPTERS = {
  anthropic: chat,
  openai: chat2,
  gemini: chat3
};
async function chat4(request) {
...
function sanitizeFilename(name) {
  ...
  const cleaned = String(name).replace(/[\\/]+/g, "/").split("/").filter((part) => part && part !== "." && part !== "..").join("/").replace(/[\u0000-\u001f<>:"|?*]/g, "_").slice(0, 180);
...
function assertAllowed(url, config, what) {
  ...
  if (isHostBlocked(host, config.blockedHosts)) {
    throw new Error(
      `Blocked by the user's settings: ${host} is on the blocked-sites list, so ${what} is not allowed here. Tell the user rather than trying another route to the same site.`
    );
  }
}
async function currentTab(ctx, { checkBlocklist = true } = {}) {
  const tab = await resolveTab(ctx.session);
...
var TOOLS = [
  {
    name: "read_page",
    description: "Read the current tab: its URL, title, scroll position, every interactive element with the index used by click/type_text, and the visible text. Use this when you need to see the page before acting, or to re-sync after the page changed on its own.",
  ...
  {
    name: "navigate",
    description: "Load a URL in the tab you are working in. Use a full URL; a bare domain gets https:// added.",
    schema: {
      type: "object",
  ...
  {
    name: "click",
    description: "Click an element by its index from the most recent page snapshot. Indices change whenever the page changes, so always use the newest list.",
    schema: {
      type: "object",
    ...
    async run({ index }, ctx) {
      const tab = await currentTab(ctx);
      const result = await clickElement(tab.id, index);
      await waitForLoad(tab.id, 8e3);
  ...
  {
    name: "type_text",
    description: "Type into a text field, textarea or contenteditable by index. For a <select>, pass the option's visible text or value. Set submit=true to press Enter afterwards (how you run a search).",
    schema: {
      type: "object",
      properties: {
    ...
    async run({ index, text, submit }, ctx) {
      const tab = await currentTab(ctx);
      const result = await typeIntoElement(tab.id, index, text, !!submit);
      if (submit) await waitForLoad(tab.id, 8e3);
  ...
  {
    name: "scroll",
    description: "Scroll the page. Use this when the element list says elements are off-screen, or to load more of an infinite feed.",
    schema: {
      type: "object",
      properties: {
    ...
    async run({ direction = "down", amount }, ctx) {
      const tab = await currentTab(ctx);
      const result = await scrollPage(tab.id, direction, amount);
      await sleep(400);
  ...
  },
  {
    ...
    async run({ index }, ctx) {
      const tab = await currentTab(ctx);
      await scrollToElement(tab.id, index);
      await sleep(300);
  ...
  {
    name: "wait",
    description: "Pause before looking again. Use for content that appears after a delay, not as a substitute for reading the page.",
    schema: {
      type: "object",
  ...
  {
    name: "download_url",
    description: "Download a file to the computer's Downloads folder from a direct URL. Use this for PDFs, images, spreadsheets and other files whose URL you already know.",
    schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Download the file a link or image on the page points at, by element index. Use this when the page shows a download link rather than a URL you can read.",
    schema: {
      type: "object",
      properties: {
    ...
    async run({ index, filename }, ctx) {
      const tab = await currentTab(ctx);
      const { href, suggestedName } = await hrefOfElement(tab.id, index);
  ...
  },
  {
    ...
    async run({ limit: limit2 = 5 }) {
      const items = await chrome.downloads.search({
        limit: Math.min(Math.max(limit2, 1), 20),
        orderBy: ["-startTime"]
      });
      if (!items.length) return { text: "No downloads recorded." };
      const lines = items.map(
        (i) => `#${i.id} ${i.state}${i.error ? ` (${i.error})` : ""} \u2014 ${i.filename || i.url}` + (i.totalBytes ? ` [${Math.round(i.bytesReceived / i.totalBytes * 100)}%]` : "")
      );
...
${lines.join("\n")}` };
  ...
  {
    name: "screenshot",
    description: "Capture the visible part of the tab as an image. Use it when the text and element list are not enough \u2014 layout, charts, images, or a page you cannot make sense of otherwise.",
    schema: { type: "object", properties: {}, additionalProperties: false },
    vision: true,
    async run(_input, ctx) {
      const tab = await currentTab(ctx);
...
var Agent = class {
  ...
  setRunning(running) {
    ...
    this.keepAlive = running ? setInterval(() => chrome.runtime.getPlatformInfo(), 2e4) : null;
  }
  stop() {
...
${await this.pageStateText(config)}`;
  ...
  async pageStateText(config) {
    try {
      const tab = await resolveTab(this.session);
...
var agent = new Agent({
  ...
  requestApproval: ({ tool, input }) => new Promise((resolve2, reject) => {
    if (!panelPort) {
      reject(new Error("Approval required but the side panel is closed"));
      return;
    }
    const id = `approval-${++approvalSeq}`;
    pendingApprovals.set(id, { resolve: resolve2, reject });
...
var MENU_PAGE = "browser-agent-page";
var MENU_SELECTION = "browser-agent-selection";
var MENU_LINK = "browser-agent-link";
...
globalThis.__browserAgentInternals = { agent, browser: browser_exports, findTool, TOOLS };
